...
load('data\Models.mat');%please use 'load('data/Models.mat')' under linux
struct=Models{5,1};
basicE=Models{4,1};
...
if strfind(filename,'.fa') || strfind(filename,'.fasta')
    ...
    save('data\test_seqs.mat','test_seqs','cut_fea');
    % call the xgboost
    system(['python xgboost_predict.py ',num2str(pretype),' ',num2str(featype)]);
    % load feautures and models, if under linux os, please replace '\' with '/'
    test_fea_file=['test_feas\test_fea',num2str(pretype),'_',num2str(featype),'.csv'];
    xgboost_file_inde_score=['predict_scores\xgboost_pre_score',num2str(pretype),'_',num2str(featype),'.csv'];
