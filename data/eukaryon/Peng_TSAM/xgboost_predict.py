...
#    pretype = int('1')
    ...
    if featype==1:
        ...
        if type(pos_fea1[0]) == np.float64:
            pos_fea1 = pos_fea1.reshape(1,pos_fea1.shape[0])
        if type(pos_fea2[0]) == np.float64:
            pos_fea2 = pos_fea2.reshape(1,pos_fea2.shape[0])
    ...
    ###### save features #########
    file_path='test_feas/test_fea'+str(pretype)+'_'+str(featype)+'.csv'
    with open(file_path,'w') as csv_file:
        writer = csv.writer(csv_file, delimiter=',')
        for line in fea_apply:
            writer.writerow(line)
            
    modelfile='models/finalized_model_'+str(pretype)+'_'+str(featype)+'.model'
    ...
    file_path='predict_scores/xgboost_pre_score'+str(pretype)+'_'+str(featype)+'.csv'    
