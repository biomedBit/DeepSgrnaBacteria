%% test pHMM
function phmm=PhmmTest(train_seq, train_score, test_seq, t, au, ad)
basicE=cell(4,1);
basicE{1,1}='A';
basicE{2,1}='C';
basicE{3,1}='G';
...
tic
[train_feature,test_feature]=multipHMMfea(train_seq,test_seq,train_score,t,au,ad,basicE);
toc
%[tr_tr,et_tr,alpStat_tr]=hmmparak(train_seq,t,au,ad,basicE);
%[tr_te,et_te,alpStat_te]=hmmparak(test_seq,t,au,ad,basicE);
%lops=[];
% for i= 1:length(test_seq)
%     [STATES,LOPp1] = hmmviterbi1(alpStat_te(i,:),tr_tr,et_tr);
%     lops=[lops;LOPp1];
% end
% phmm=lops;
