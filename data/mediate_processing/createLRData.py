...
def synthetize_feature(SET,ENZ,cv):
	path = "../Cross_validation/" + SET + "/" +ENZ + "/" + cv + "/"
	net = CNN_5()
	modelName =""
	for model in os.listdir(path):
		if 'pkl' in model:
			modelName = model 
	net.load_state_dict(torch.load(path + modelName))
	net.cuda()	 
	net = net.eval()

	fw = open(path + 'traindata_result.txt','w')		
	with open(path + 'traindata.txt','r') as fr:
		...
		for line in tqdm(linelist):
			...
			line = fr.readline()
	fw.close()
	fw = open(path + 'testdata_result.txt','w')		
	with open(path + 'testdata.txt','r') as fr:
		linelist = fr.readlines()
		for line in tqdm(linelist):
			linelist = line.strip('\n').split("\t")
			asample = get_numpy_4_43_1(linelist)	
...
synthetize_feature("Set1","Cas9","1")
synthetize_feature("Set1","Cas9","2")
synthetize_feature("Set1","Cas9","3")
synthetize_feature("Set1","Cas9","4")
synthetize_feature("Set1","Cas9","5")

synthetize_feature("Set1","eSpCas9","1")
synthetize_feature("Set1","eSpCas9","2")
synthetize_feature("Set1","eSpCas9","3")
synthetize_feature("Set1","eSpCas9","4")
synthetize_feature("Set1","eSpCas9","5")

synthetize_feature("Set1","knoRecA_Cas9","1")
synthetize_feature("Set1","knoRecA_Cas9","2")
synthetize_feature("Set1","knoRecA_Cas9","3")
synthetize_feature("Set1","knoRecA_Cas9","4")
synthetize_feature("Set1","knoRecA_Cas9","5")
